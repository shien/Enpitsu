...
> **状態: ⚠️ 一部完了（2026-07 時点）**
> 候補の内部管理（`src/candidate.rs` の `CandidateList`）・キー操作→コマンド変換・
> composition へのインライン表示は実装済み。一方、当初計画した **ポップアップ候補
> ウィンドウ**（`candidate_window.rs` / `ITfCandidateListUIElement`）は未実装で、
> 現状は SKK 的にインラインで候補を切り替えて確定する方式で代替している。
> 未対応: 5.1 ウィンドウ作成 / 5.4 見た目・DPI 対応。
